function initNav(permissions) {
    ...
    if (permissions) {
        const stairMenus = getChildArr(permissions, "0");
        stairMenus.forEach(function (stairMenu) {
            $nav.append(createLi(permissions, stairMenu))
        })
    }
}
...
function createLi(permissions, permission) {
    ...
    if (childArr.length > 0) {  // 有子菜单
        ...
        const a = document.createElement('a'); // 生成一个 a 元素
        a.setAttribute("onclick", "xadmin.add_tab('" + permission.name + "', '" +
            Constant.API.BASE.URL + permission.operationUrl + "')");
        const iconfont = document.createElement('i'); // 生成一个 i 元素
        iconfont.setAttribute("class", "iconfont");
        $(iconfont).html("&#xe6a7;");
        a.appendChild(iconfont);
        a.appendChild(cite);
        li.appendChild(a);
    }
